...
#create a thumb .gif from a file
def gen_thumb(filename, out_path, filemime, id_mg, start_mg, ffmpeg, self, myos, thumbsave):
    newthumb = "%s%s%d%s" % (id_mg, "-", start_mg, ".gif")
    aud_pat=str(os.path.dirname(os.path.abspath(__file__))+"/default/audio.gif")
    err_pat=str(os.path.dirname(os.path.abspath(__file__))+"/default/error.gif")
    if filemime == 3:
        shutil.copy(aud_pat, os.path.join(out_path, newthumb))
    else:
        path_thumbsave = os.path.join(os.path.dirname(filename), ".mediagrid", "%s%s" % (os.path.splitext(os.path.basename(filename))[0], ".gif"))
        if (thumbsave == 1) and (os.path.isfile(path_thumbsave)):
            ...
            if filemime == 1:
                ...
                commandln = [ffmpeg, "-ss", humanize_time(middle), "-vframes", "1", "-i", filename, "-vcodec", "gif", "-f", "image2", "-s", "60x40", os.path.join(out_path, newthumb)]
            if filemime == 2:
                commandln = [ffmpeg, "-i", filename, "-vcodec", "gif", "-f", "image2", "-s", "60x40", os.path.join(out_path, newthumb)]
            try:
                startupinfo = None
                if myos == "Windows":
